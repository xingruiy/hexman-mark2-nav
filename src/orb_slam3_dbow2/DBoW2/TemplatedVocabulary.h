...
public:
  ...
  /**
   * Loads the vocabulary from a compact binary file produced by
   * saveToBinaryFile(). Returns false if the file is missing, has the
   * wrong magic/version, or is otherwise malformed.
   * @param filename
   */
  bool loadFromBinaryFile(const std::string &filename);

  /**
   * Saves the vocabulary into a compact binary file. The layout is
   * documented above loadFromBinaryFile in the .h implementation.
   * @param filename
   */
...
//   int32_t   m_scoring
//   int32_t   m_weighting
//   int32_t   F_L                 // descriptor byte count
//   uint64_t  n_nodes             // excludes root, matches text format
//   per non-root node, in same order as the text file:
//     int32_t   parent_id
//     uint8_t   is_leaf
//     uint8_t   descriptor[F_L]
//     double    weight
//
// Assumes the descriptor of F is a cv::Mat-like buffer of F::L bytes
// (true for FORB, the only descriptor used by ORB-SLAM3).
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::saveToBinaryFile(const std::string &filename) const
{
  std::ofstream f(filename.c_str(), std::ios::binary);
  if(!f) throw string("Could not open file ") + filename;

  const char magic[8] = {'D','B','o','W','2','B','I','N'};
  const uint32_t version   = 1;
  const int32_t  k         = static_cast<int32_t>(m_k);
  const int32_t  L         = static_cast<int32_t>(m_L);
  const int32_t  scoring   = static_cast<int32_t>(m_scoring);
  const int32_t  weighting = static_cast<int32_t>(m_weighting);
  const int32_t  F_L       = static_cast<int32_t>(F::L);
  const uint64_t n_nodes   = m_nodes.empty() ? 0u
                                             : static_cast<uint64_t>(m_nodes.size() - 1);
  ...
  f.write(reinterpret_cast<const char*>(&F_L),       sizeof(F_L));
  f.write(reinterpret_cast<const char*>(&n_nodes),   sizeof(n_nodes));

  for(size_t i = 1; i < m_nodes.size(); ++i)
  {
    const Node &node = m_nodes[i];
    const int32_t parent_id = static_cast<int32_t>(node.parent);
    const uint8_t is_leaf   = node.isLeaf() ? 1u : 0u;
    const double  weight    = static_cast<double>(node.weight);

    f.write(reinterpret_cast<const char*>(&parent_id), sizeof(parent_id));
    f.write(reinterpret_cast<const char*>(&is_leaf),   sizeof(is_leaf));
    f.write(reinterpret_cast<const char*>(node.descriptor.ptr()), F::L);
...
bool TemplatedVocabulary<TDescriptor,F>::loadFromBinaryFile(const std::string &filename)
{
  ...
  char magic[8];
  f.read(magic, 8);
  if(!f || std::memcmp(magic, "DBoW2BIN", 8) != 0)
  {
    std::cerr << "Vocabulary loading failure: bad magic in " << filename << std::endl;
    return false;
  }
  ...
  if(!f || version != 1u)
  {
    std::cerr << "Vocabulary loading failure: unsupported binary version "
              << version << " in " << filename << std::endl;
    return false;
  }

  int32_t k = 0, L = 0, scoring = 0, weighting = 0, F_L = 0;
  uint64_t n_nodes = 0;
  ...
  f.read(reinterpret_cast<char*>(&F_L),       sizeof(F_L));
  f.read(reinterpret_cast<char*>(&n_nodes),   sizeof(n_nodes));
  if(!f || F_L != static_cast<int32_t>(F::L))
  {
    std::cerr << "Vocabulary loading failure: descriptor length mismatch ("
              << F_L << " vs " << F::L << ") in " << filename << std::endl;
    return false;
  }

  m_k         = static_cast<int>(k);
  m_L         = static_cast<int>(L);
  m_scoring   = static_cast<ScoringType>(scoring);
  m_weighting = static_cast<WeightingType>(weighting);
  createScoringObject();

  m_words.clear();
  m_nodes.clear();
  m_nodes.reserve(n_nodes + 1);
  m_words.reserve(n_nodes);
  m_nodes.resize(1);
  m_nodes[0].id = 0;
  ...
  for(uint64_t i = 0; i < n_nodes; ++i)
  {
    ...
    f.read(reinterpret_cast<char*>(&is_leaf),   sizeof(is_leaf));
    int nid = static_cast<int>(m_nodes.size());
    m_nodes.resize(m_nodes.size() + 1);
    m_nodes[nid].id          = nid;
    m_nodes[nid].parent      = static_cast<NodeId>(parent_id);
    m_nodes[parent_id].children.push_back(nid);
    m_nodes[nid].descriptor.create(1, F::L, CV_8U);
    f.read(reinterpret_cast<char*>(m_nodes[nid].descriptor.ptr()), F::L);
    f.read(reinterpret_cast<char*>(&weight), sizeof(weight));
    if(!f)
    {
      std::cerr << "Vocabulary loading failure: truncated binary file "
                << filename << std::endl;
      m_words.clear();
      m_nodes.clear();
      return false;
    }
    m_nodes[nid].weight = static_cast<WordValue>(weight);
