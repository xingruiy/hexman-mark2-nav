...
static void PrintSystemBanner(System::eSensor sensor)
{
    cout << endl <<
    ...
    switch(sensor)
    {
        case System::MONOCULAR:     cout << "Monocular" << endl;          break;
        case System::STEREO:        cout << "Stereo" << endl;             break;
        case System::RGBD:          cout << "RGB-D" << endl;              break;
        case System::IMU_MONOCULAR: cout << "Monocular-Inertial" << endl; break;
        case System::IMU_STEREO:    cout << "Stereo-Inertial" << endl;    break;
        case System::IMU_RGBD:      cout << "RGB-D-Inertial" << endl;     break;
    }
}

ORBVocabulary* System::LoadVocabularyFromPath(const string &strVocFile)
{
    cout << endl << "Loading ORB Vocabulary. This could take a while..." << endl;

    ORBVocabulary* pVoc = new ORBVocabulary();

    // ".bin" suffix -> fast binary loader; anything else -> text loader.
    const bool useBinary = strVocFile.size() >= 4 &&
        strVocFile.compare(strVocFile.size() - 4, 4, ".bin") == 0;

    const bool bVocLoad = useBinary
        ? pVoc->loadFromBinaryFile(strVocFile)
        : pVoc->loadFromTextFile(strVocFile);
...
System::System(const string &strVocFile, Settings* settings, const eSensor sensor,
               const int initFr, const string &strSequence):
    mSensor(sensor), mbReset(false), mbResetActiveMap(false),
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    PrintSystemBanner(mSensor);

    mStrVocabularyFilePath = strVocFile;
    mpVocabulary = LoadVocabularyFromPath(strVocFile);

    InitFromSettings(settings, initFr, strSequence);
}

System::System(ORBVocabulary* pVocabulary, const string &strVocFilePath,
               Settings* settings, const eSensor sensor,
               const int initFr, const string &strSequence):
    mSensor(sensor), mbReset(false), mbResetActiveMap(false),
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    if(pVocabulary == nullptr)
    {
        cerr << "System(): pre-loaded vocabulary pointer is null." << endl;
        exit(-1);
    }
    mStrVocabularyFilePath = strVocFilePath;
    mpVocabulary = pVocabulary;
    cout << "Using pre-loaded ORB vocabulary (" << mpVocabulary->size()
         << " words)." << endl;

    InitFromSettings(settings, initFr, strSequence);
}

void System::InitFromSettings(Settings* settings,
                               const int initFr,
                               const string &strSequence)
{
    ...
    mpTracker = new Tracking(this, mpVocabulary,
                             mpAtlas, mpKeyFrameDatabase, mSensor, settings_, strSequence);
...
void System::SetKeyFrameInsertedCallback(KeyFrameInsertedCallback cb)
{
    unique_lock<mutex> lock(mMutexKeyFrameCb);
    mKeyFrameInsertedCb = std::move(cb);
}

void System::NotifyKeyFrameInserted()
{
    KeyFrameInsertedCallback cb;
    {
        unique_lock<mutex> lock(mMutexKeyFrameCb);
        cb = mKeyFrameInsertedCb;
    }
    if (!cb || !mpTracker) return;
