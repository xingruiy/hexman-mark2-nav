...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const int sensor, Settings* settings, const string &_nameSeq):
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    {
        if(mbRGB)
            cvtColor(imRGB,imGray,cv::COLOR_RGB2GRAY);
        else
            cvtColor(imRGB,imGray,cv::COLOR_BGR2GRAY);
    }
    ...
    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(imGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(imGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if (mpSystem) mpSystem->NotifyKeyFrameInserted();
}
